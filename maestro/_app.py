...
from maestro.integrations._home_assistant.websocket_manager import WebSocketManager
from maestro.triggers._cron import CronTriggerManager
from maestro.triggers._maestro import MaestroEvent, MaestroTriggerManager
from maestro.triggers._sun import SunTriggerManager
from maestro.utils import _internal
    ...
    def __init__(
        ...
        *,
        ...
        if configure_logging:
            _internal.configure_logging()

        if config.db_url is not None:
            self._initialize_db()

        self._add_project_paths()
        _internal.import_custom_domains()
        _internal.load_script_modules(config.scripts_dir)
